...
def main():
    ...
    for clf_info in clf_files:
        try:
            ...
            # Process each shape in the layer - only look for holes in Skin folders
            if hasattr(layer, 'shapes') and len(layer.shapes) >= 2:
                # Only check for holes in folders that contain "Skin"
                ...
                if can_have_holes:
                    ...
                    if hasattr(shape1, 'points') and shape1.points and len(shape1.points) > 0:
                        ...
                        if len(hole_path) >= 3:  # Valid path with at least 3 points
                            ...
                            print(f"  Found hole: Shape[1] Path[0] with {len(hole_path)} points in {folder_name}")
